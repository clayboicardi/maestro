...
class ConfigStager:
    ...
    1. **Baseline cache** — on first :meth:`modify` call (or any path that
       reads the staged config), :meth:`_ensure_baseline` fetches the
       current AIOStreams config via the injected ``get_config`` callback
       and stores both the baseline (immutable reference) and a deep-copy
       working state (``_staged``). Subsequent :meth:`modify` calls operate
       on ``_staged`` without re-fetching.
    2. **Stage** — each :meth:`modify` call applies a ``TransformFn`` to a
       fresh deep-copy of ``_staged``, records a :class:`PendingMutation`
       describing the change, and replaces ``_staged`` with the result.
    3. **Flush** — :meth:`save` issues a single PUT with ``_staged`` and,
       on success, clears baseline/staged/mutations so the next
       :meth:`modify` re-fetches.

    Failure semantics:

    - If ``put_config`` raises during :meth:`save`, staged state survives.
      The caller may retry :meth:`save` without re-staging, or call
      :meth:`invalidate_cache` to drop state and start fresh against the
      latest server-side baseline.
    - :meth:`save` is idempotent for the no-mutations case: returns
      ``{"ok": True, "no_changes": True}`` without issuing a PUT.

    Concurrency: not thread-safe and not coroutine-safe. One stager per
    user session; FastMCP serializes tool invocations within a session.
    """
    ...
    async def save(self) -> dict[str, Any]:
        ...
        Returns ``{"ok": True, "no_changes": True}`` if no mutations are
        staged (no PUT issued). Otherwise returns ``{"ok": True,
        **put_response, "changes_applied": [<field>, ...]}`` where
        ...
        state is preserved -- callers may retry :meth:`save` directly or
        call :meth:`invalidate_cache` to discard and restart against the
        latest server-side baseline.
        """
